...
function getStringLength(value) {
  return value == null || undefined ? 0 : value.length;
}
...
function formatTime(minutes, seconds) {
  return seconds.toString().padStart(5, '0'.concat(minutes, ':'));
}
...
function isPalindrome(str) {
  const solidStr = str.replace(/[\W_]/g, '');
  const lowStr = solidStr.toLowerCase();
  const firstHalf = lowStr.substring(0, solidStr.length / 2);
  const secondHalf = lowStr.substring(
    Math.ceil(solidStr.length / 2),
    solidStr.length
  );
  const secondArr = secondHalf.split('');
  const secondArrReverse = secondArr.reverse();
  const secondHalfJoin = secondArrReverse.join('');
  return firstHalf === secondHalfJoin;
}
